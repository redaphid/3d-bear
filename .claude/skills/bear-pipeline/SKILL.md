...
| 4 hollow | `python tools/hollow.py <solid>.stl --out print-ready/totem-<bear>-hollow-2mm.stl --wall 2.0 --pitch 0.5 --height 160 --outer keep --views` (uniform-wall shell, outer surface kept as-is, base open; slice as a normal solid). **For a big print build the solid first with `mesh_repair.py --pitch 0.3 --faces 250000 --smooth 3`** — detail scales with pitch and face budget, and the default recipe leaves a 240 mm bear faceted | the totem STL, ~100 g at 160 / ~240 g at 240 |
| 5 keychain | `python tools/keychain.py print-ready/<bear>.stl --out print-ready/keychain-<bear>-32mm.stl --scale 0.2 --views` (loop on the head, manifold union, solid print); `--style hole --through head --nfc` for the cord tunnel + NTAG215 pocket under the plinth | 32 mm charm |
